...
import InterviewQuestionModal from "../CareerComponents/InterviewQuestionModal";
...
export default function (props) {
  ...
  async function fetchInstructionPrompt() {
    try {
      ...
      const prompt= configData?.question_gen_prompt.prompt;
      if (!prompt) {
        console.warn('question_gen_prompt missing from global settings', configData);
        return;
      }

      setQuestionGenPrompt(prompt);
    } catch (e) {
      console.log("[Question Generator Fetch Prompt Error]", e);
    }
  }
